...
describe("preprocessContextReferences", () => {
  test("expands file references into the message body", async () => {
    ...
    const result = await preprocessContextReferences("Summarize @file:notes.txt:2-3", {
      cwd: workspaceDir,
    });
  ...
  test("expands folder references into a tree listing", async () => {
    ...
    const result = await preprocessContextReferences("Inspect @folder:src", { cwd: workspaceDir });
  ...
  test("warns when a file reference escapes the active workspace", async () => {
    ...
    try {
      ...
      const result = await preprocessContextReferences(`Read @file:${outsideFile}`, {
        cwd: workspaceDir,
        allowedRoot: workspaceDir,
      });
