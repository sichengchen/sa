...
- **claude_code**: Checks for OAuth session (`claude auth status`), falls back to `ANTHROPIC_API_KEY` from Aria secrets
- **codex**: Checks for auth, falls back to `OPENAI_API_KEY` from Aria secrets
...
- **No interactive mode** — Esperta Aria cannot pipe stdin to subprocesses
