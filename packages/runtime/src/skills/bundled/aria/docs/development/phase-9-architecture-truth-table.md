...
| `@aria/server` | `hybrid target shell` | `packages/server/src/*` | `@aria/runtime`, `@aria/gateway`, and the `aria` CLI boot path | The target server composition root plus daemon/discovery lifecycle shell are live, but the legacy CLI launch path still has to stay stable. |
