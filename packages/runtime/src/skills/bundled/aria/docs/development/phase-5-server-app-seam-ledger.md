...
| --- | --- | --- | --- |
| `@aria/server` | `packages/server/src/{app,brand,daemon,discovery,engine}.ts` plus `packages/gateway/src/server.ts` and `@aria/runtime` composition | Server bootstrap helpers, runtime + gateway composition root, runtime-home/brand helpers, discovery-file lifecycle coordination, and a package-owned Aria Server entry surface | `@aria/runtime`, `@aria/gateway`, `@aria/shared-types`, and the current `aria` CLI boot path |
