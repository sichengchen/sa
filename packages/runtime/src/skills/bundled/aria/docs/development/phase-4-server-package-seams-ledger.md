...
| --- | --- | --- | --- |
| `@aria/projects` | `packages/projects-engine/src/{repository,publish,reviews,external-refs}.ts` plus tracked-work materialization in `packages/handoff/src/service.ts`; planning/blocker coordination now lives in `packages/projects/src/*` | Project registry, task/thread/dispatch/review/publish coordination, project-thread orchestration APIs | `@aria/projects-engine`, `@aria/handoff`, and current `aria projects` command names |
