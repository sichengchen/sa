...
import type { DispatchExecutionEvent } from "../../projects-engine/src/bridge.js";
...
function buildDispatchPrompt(repository: ProjectsEngineRepository, dispatchId: string): string {
  ...
  const task = dispatch.taskId ? repository.getTask(dispatch.taskId) : thread?.taskId ? repository.getTask(thread.taskId) : undefined;
...
function mapResultToEvent(dispatchId: string, executionSessionId: string, result: { status: string; summary?: string | null; stderr?: string | null }): DispatchExecutionEvent {
