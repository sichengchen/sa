...
export class SessionArchiveManager {
  ...
  getMessages(sessionId: string): ArchivedMessage[] {
    if (!this.db) return [];
    const stmt = this.db.prepare(`
  ...
  async listRecent(limit = 20): Promise<ArchivedSessionRecord[]> {
    return this.listArchived(limit);
  }

  search(query: string, limit = 10): ArchivedSessionSearchResult[] {
    if (!this.db) return [];
    ...
    return stmt.all(sanitized, limit).map((row: any) => ({
      sessionId: row.session_id,
      connectorType: row.connector_type,
      connectorId: row.connector_id,
      createdAt: row.created_at,
