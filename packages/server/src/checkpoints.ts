...
import { toRelativeIfInside } from "@aria/policy/path-boundary";
...
export class CheckpointManager {
  ...
  private async takeSnapshot(workingDir: string, reason: string): Promise<boolean> {
    ...
    const log = this.runGit(
      ["rev-list", "--max-count", String(this.maxSnapshots), "HEAD"],
      shadowRepo,
      absDir,
    );
    if (log.ok) {
  ...
  async listCheckpoints(workingDir: string): Promise<CheckpointEntry[]> {
    ...
    const log = this.runGit(
      ["log", "--format=%H|%h|%aI|%s", "-n", String(this.maxSnapshots)],
      shadowRepo,
      absDir,
    );
  ...
  async diff(
    ...
    if (!diff.ok && !diff.stdout) {
      return {
        success: false,
        error: diff.stderr || "Failed to diff checkpoint.",
      };
    }
...
export function checkpointWorkdirForArgs(
  toolName: string,
  ...
  if (toolName === "exec") {
    ...
    if (typeof command === "string" && shouldCheckpointExec(command)) {
      const workdir =
        typeof args.workdir === "string" && args.workdir.trim() ? args.workdir : fallbackWorkingDir;
