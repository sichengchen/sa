...
| --- | --- | --- | --- |
| `@aria/server` | `packages/server/src/{app,daemon,engine}.ts` plus `packages/gateway/src/server.ts` and `@aria/runtime` composition | Server bootstrap helpers, runtime + gateway composition root, daemon lifecycle coordination, and a package-owned Aria Server entry surface | `@aria/runtime`, `@aria/gateway`, and the current `aria` CLI boot path |
...
- `packages/server/src/{daemon,engine}.ts` now own the daemon bootstrap and lifecycle shell; keep any remaining runtime compatibility wrappers thin.
