...
| `@aria/projects` | `legacy-owned seam` | `packages/projects-engine/src/{repository,store,schema,types}.ts` with planning/review/publish/external-ref coordination now in `packages/projects/src/*` | `packages/projects/src/*` | The target package now owns most project-control coordination, but the durable repository/store core still comes from `projects-engine`. |
