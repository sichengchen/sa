...
| `@aria/projects` | `legacy-owned seam` | `packages/projects-engine/src/{repository,store,external-refs,schema,types}.ts` with planning/review/publish coordination now in `packages/projects/src/*` | `packages/projects/src/*` | The target package now owns planning/review/publish coordination, but the durable project-control core still comes from `projects-engine`. |
...
| `@aria/projects-engine` | `legacy-only compatibility surface` | Project registry, planning, reviews, publish, and tracked-work persistence plus compatibility wrappers for workspace/job seams | `@aria/projects`, `@aria/workspaces`, `@aria/jobs` | This still owns most project-control behavior, but workspace and jobs slices should keep shrinking behind compatibility wrappers. |
