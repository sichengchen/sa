...
| `@aria/projects` | `legacy-owned seam` | `packages/projects-engine/src/{repository,store,reviews,publish,external-refs,schema,types}.ts` with planning/blocker coordination now in `packages/projects/src/*` | `packages/projects/src/*` | The target package now owns planning/blocker coordination, but the durable project-control core still comes from `projects-engine`. |
