...
| --- | --- | --- | --- | --- |
| `@aria/protocol` | `target-owned` | `packages/protocol/src/*` | direct target-package imports | Phase 1 moved the protocol contracts under the target package name and current production code now imports them directly. |
...
| `@aria/agents-coding` | `target-owned` | `packages/agents-coding/src/*` | direct target-package imports | The target seam now owns the shared backend contracts and concrete adapter implementations. |
| `@aria/access-client` | `target-owned` | `packages/access-client/src/*` | target package type inputs | Client-facing access helpers are already target-owned. |
...
| `@aria/server` | `hybrid target shell` | `packages/server/src/*` | `@aria/runtime`, `@aria/gateway`, and the `aria` CLI boot path | The target server composition root plus brand/daemon/discovery lifecycle shell are live, but the legacy CLI launch path still has to stay stable. |
...
| `@aria/mobile` | `hybrid target shell` | `packages/mobile/src/*` | `apps/aria-mobile`, `@aria/access-client`, `@aria/ui`, and `@aria/projects` | The mobile shell is real and remains remote-first, but it still composes shared client/project seams during migration. |
| `apps/aria-desktop` | `hybrid target shell` | `apps/aria-desktop/*` | `@aria/desktop`, `@aria/access-client`, `@aria/ui`, and `@aria/projects` | The target desktop app surface is live; edit it as the desktop shell entrypoint while preserving the listed shared client seams. |
| `apps/aria-mobile` | `hybrid target shell` | `apps/aria-mobile/*` | `@aria/mobile`, `@aria/access-client`, `@aria/ui`, and `@aria/projects` | The target mobile app surface is live; edit it as the mobile shell entrypoint while preserving the listed shared client seams. |
