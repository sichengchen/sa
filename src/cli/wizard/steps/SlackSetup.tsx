...
export function SlackSetup({ onNext, onBack, currentValues }: SlackSetupProps) {
  ...
  useInput((input, key) => {
    ...
    if (phase === "token") {
      ...
      if (key.return) {
        if (!token) { onNext({ slackToken: "", slackSigningSecret: "" }); return; }
      ...
      if (key.return) { onNext({ slackToken: token, slackSigningSecret: secret }); return; }
  ...
  if (phase === "keep-or-change" && currentValues) {
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold color="cyan">Slack Bot Setup</Text>
        <Text />
        <Text>Current configuration:</Text>
        <Text>  Bot token: {currentValues.slackToken ? "configured" : "not configured"}</Text>
        <Text>  Signing secret: {currentValues.slackSigningSecret ? "configured" : "not configured"}</Text>
        <Text />
  ...
  if (phase === "secret") {
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold color="cyan">Slack Signing Secret</Text>
        <Text />
