...
export function GitHubSetup({ onNext, onBack, currentValues }: GitHubSetupProps) {
  ...
  useInput((input, key) => {
    if (phase === "keep-or-change") {
      ...
      if (input?.toLowerCase() === "c") { setPhase("token"); return; }
      return;
    }
    ...
    if (phase === "token") {
      ...
      if (key.return) {
        if (!token) { onNext({ githubToken: "", githubWebhookSecret: "" }); return; }
        setPhase("secret"); return;
      }
      if (key.backspace || key.delete) { setToken((v) => v.slice(0, -1)); return; }
      if (input && !key.ctrl && !key.meta) { setToken((v) => v + input); }
    } else if (phase === "secret") {
      if (key.escape) { setPhase("token"); return; }
      if (key.return) { onNext({ githubToken: token, githubWebhookSecret: secret }); return; }
      if (key.backspace || key.delete) { setSecret((v) => v.slice(0, -1)); return; }
      if (input && !key.ctrl && !key.meta) { setSecret((v) => v + input); }
    }
  });
  ...
  if (phase === "keep-or-change" && currentValues) {
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold color="cyan">GitHub Bot Setup</Text>
        <Text />
        <Text>Current configuration:</Text>
        <Text>  Token: {currentValues.githubToken ? "configured" : "not configured"}</Text>
        <Text>  Webhook secret: {currentValues.githubWebhookSecret ? "configured" : "not configured"}</Text>
        <Text />
  ...
  if (phase === "secret") {
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold color="cyan">GitHub Webhook Secret</Text>
        <Text />
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <Text />
      <Text>To use Esperta Aria via GitHub (issue/PR mentions), create a GitHub App or use a PAT.</Text>
