...
export function LinearSetup({ onNext, onBack, currentValues }: LinearSetupProps) {
  ...
  useInput((input, key) => {
    ...
    if (phase === "key") {
      ...
      if (key.return) {
        if (!apiKey) { onNext({ linearApiKey: "", linearWebhookSecret: "" }); return; }
      ...
      if (key.escape) { setPhase("key"); return; }
      if (key.return) { onNext({ linearApiKey: apiKey, linearWebhookSecret: secret }); return; }
  ...
  if (phase === "keep-or-change" && currentValues) {
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold color="cyan">Linear Bot Setup</Text>
        <Text />
        <Text>Current configuration:</Text>
        <Text>  API key: {currentValues.linearApiKey ? "configured" : "not configured"}</Text>
        <Text>  Webhook secret: {currentValues.linearWebhookSecret ? "configured" : "not configured"}</Text>
        <Text />
  ...
  if (phase === "secret") {
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold color="cyan">Linear Webhook Secret</Text>
        <Text />
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <Text />
      <Text>To use Esperta Aria via Linear (issue mentions), generate an API key at linear.app/settings/api.</Text>
